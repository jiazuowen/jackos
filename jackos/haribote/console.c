...
void console_task(struct SHEET *sheet, unsigned int memtotal) 
{
	...
	/*这里禁止使用return，因为这不是函数调用, 和HariMain一样*/
}
...
void cmd_exit(struct CONSOLE *cons, int *fat)
{
	...
	struct FIFO32 *fifo = (struct FIFO32 *) *((int *) 0x0fec);	/*task_a的FIFO*/
...
int cmd_app(struct CONSOLE *cons, int *fat, char *cmdline)
{
	...
	if (finfo != 0) {
		...
		if (appsize >= 36 && strncmp(p+4, "Hari", 4) == 0 && *p == 0x00) {
			...
			task->ds_base = (int) q;
			set_segmdesc(task->ldt + 0, finfo->size - 1, (int) p, AR_CODE32_ER + 0x60); /*可执行代码段*/
			set_segmdesc(task->ldt + 1, segsiz - 1,   	 (int) q, AR_DATA32_RW + 0x60); /*数据段*/
			for (i = 0; i < datsiz; i++) {
				q[esp + i] = p[dathrb + i];
			}
...
int *hrb_api(int edi, int esi, int ebp, int esp, int ebx, int edx, int ecx, int eax)
{
	...
	if (edx == 1) {
		...
		for (;;) {
			...
			io_sti();
			if (i <= 1) { /*光标用定时器*/
				/*应用程序运行时不需要显示光标，因此总是将下次显示用的值设置为1*/
				timer_init(cons->timer, &task->fifo, 1); /*下次值为1*/
				timer_settime(cons->timer, 50);
			}
			if (i == 2) { /*光标ON*/
				cons->cur_c = COL8_FFFFFF;
