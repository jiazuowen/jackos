...
void HariMain(void)
{
	...
	sheet_setbuf(sht_win, buf_win, 160, 52, -1); /*透明色*/
	...
	mx = (binfo->scrnx - 16) / 2; /*安显示画面的中央来计算坐标*/
	...
	load_tr(3 * 8);
	task_b_esp = memman_alloc_4k(memman, 64*1024) + 64*1024 - 8; /*这里注意了 -8*/
	...
	tss_b.fs = 1 * 8;
	tss_b.gs = 1 * 8;
	*((int *) (task_b_esp+4)) = (int)sht_back;	// 将sht_back的地址放入栈中
	mt_init();
